...
class BotHandlers:
    ...
    def __init__(self):
        ...
        self.state_handlers = {
            ...
            UserState.AWAITING_STATS_PERIOD: self.handle_stats_period,
    ...
    def process_message(self, phone: str, text: str = None, 
        ...
        try:
            ...
            elif text == MenuCommands.STATISTICS:
                # Enter statistics period selection
                self.db.set_user_state(phone, UserState.AWAITING_STATS_PERIOD.name)
                return Messages.statistics_menu()
    ...
    def handle_stats_period(self, phone: str, text: str, temp_data: Dict) -> str:
        """Handle user selection of statistics period and return stats."""
        if not text:
            return Messages.statistics_menu()
        ...
        if cmd == '0':
            self.db.clear_user_state(phone)
            return self.show_main_menu(phone, self.db.get_driver(phone))
        ...
        if cmd == '1':
            ...
            return "Неверная опция. " + Messages.statistics_menu()

        stats = self.db.get_statistics(start_date)
        # Clear state and return formatted report
        self.db.clear_user_state(phone)
        return Messages.statistics_report(stats)
